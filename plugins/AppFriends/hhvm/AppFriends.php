...
/****** Get the clearance level of a friend ******/
	...
	(
		int $uniID			// <int> The UniID of the main user.
	,	int $friendID		// <int> The UniID of the friend to check the clearance of.
	): int					// RETURNS <int> the friend's clearance level, 0 on failure.
...
/****** Set a relationship with a friend ******/
	...
	(
		int $uniID			// <int> The User's UniID.
	,	int $friendID		// <int> The Friend's UniID.
	,	int $clearance 		// <int> The level of clearance being set.
	): bool					// RETURNS <bool> TRUE if the clearance was set properly, FALSE on failure.
...
/****** Follow a User ******/
	...
	(
		int $uniID			// <int> The UniID that's going to follow someone.
	,	int $friendID		// <int> The UniID to follow.
...
/****** Unfollow a User ******/
	...
	(
		int $uniID			// <int> The UniID that's going to unfollow someone.
	,	int $friendID		// <int> The UniID to unfollow.
	): bool					// RETURNS <bool> TRUE on success, FALSE on failure.
...
/****** Unfriend a User ******/
	...
	(
		int $uniID			// <int> The UniID that's going to unfriend someone.
	,	int $friendID		// <int> The UniID to unfriend.
	): bool					// RETURNS <bool> TRUE on success, FALSE on failure.
...
/****** Get the list of followers of a user ******/
	...
	(
		int $uniID					// <int> The UniID of the user to find followers of.
...
/****** Check if UniID has already requested friendship with FriendID (the receiver of the request) ******/
	...
	): bool					// RETURNS <bool> TRUE if there was a request, FALSE if not
...
/****** Get a list of a user's friend requests ******/
	...
	(
		int $uniID			// <int> The UniID of the user to find friend requests of.
	,	int $startPos = 0	// <int> The row number to start at.
	,	int $limit = 20		// <int> The number of rows to return.
	): array <int, array<str, mixed>>					// RETURNS <int:[str:mixed]> the list containing friend requests.
...
/****** Get a list of a user's sent friend requests ******/
	...
	(
		int $uniID			// <int> The UniID of the user to find friend requests they sent.
	,	int $startPos = 0	// <int> The row number to start at.
	,	int $limit = 20		// <int> The number of rows to return.
	): array <int, array<str, mixed>>					// RETURNS <int:[str:mixed]> the list containing friend requests.
...
/****** Send a friend request ******/
	...
	(
		int $uniID		// <int> The UniID that is requesting the friendship.
	,	int $friendID	// <int> The Friend's UniID.
	): bool				// RETURNS <bool> TRUE if the request was sent properly, FALSE on failure.
...
/****** Approve a Friend Request ******/
	...
	(
		int $uniID			// <int> The UniID of the user that is approving the request.
	,	int $friendID		// <int> the UniID of the friend that is receiving the new permissions.
	): bool					// RETURNS <bool> TRUE on success, FALSE on failure.
		...
		if($pass = Database::query("DELETE FROM friends_requests WHERE (uni_id=? AND friend_id=?) or (uni_id=? AND friend_id=?) LIMIT 1", array($uniID, $friendID, $friendID, $uniID)))
		{
			...
			if($pass = Database::query("UPDATE friends_list SET clearance=? WHERE uni_id=? AND friend_id=? LIMIT 1", array(4, $uniID, $friendID)))
			{
				if($pass = Database::query("UPDATE friends_list SET clearance=? WHERE uni_id=? AND friend_id=? LIMIT 1", array(4, $friendID, $uniID)))
				{
					...
					if($pass = Database::query("UPDATE social_data SET friends=friends+1 WHERE uni_id=? LIMIT 1", array($uniID)))
					{
						$pass = Database::query("UPDATE social_data SET friends=friends+1 WHERE uni_id=? LIMIT 1", array($friendID));
					}
				}
...
/****** Deny a friend request ******/
	...
	// AppFriends::deny($uniID, $friendID);
	{
		...
		$pass = Database::query("DELETE FROM friends_requests WHERE (uni_id=? AND friend_id=?) or (uni_id=? AND friend_id=?) LIMIT 1", array($uniID, $friendID, $friendID, $uniID));
...
/****** Track engagement you have with a target ******/
	...
	(
		int $uniID				// <int> The uniID to track.
	,	int $targetID			// <int> The uniID of the target account.
	,	int $engageValue = 1	// <int> The value of the engagement.
...
/****** Run this in the beginning of each cycle (updates all friends to recent engagement value) ******/
	...
	(
		int $uniID		// <int> The UniID of the user to update the engagement with.
	,	int $friendID	// <int> The UniID of the friend to update the engagement with.
	): bool				// RETURNS <bool> TRUE when finished, FALSE if fails.
...
/****** Get List of Suggested Friends (more likely that you may know them) ******/
	...
	): array				// RETURNS <array> full list of suggested friend matrix, or empty array if fails.
